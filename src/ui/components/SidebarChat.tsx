...
import { Send, X, MessageSquare, ShieldAlert, Award } from 'lucide-react';
import type { LogMessage } from '@/engine/model/Table';
import { useLanguageStore, t, translateSystemLog } from '@/i18n/languageStore';
    ...
    <div className="w-full h-full flex flex-col bg-slate-950/90 backdrop-blur-xl border-l border-white/5 text-slate-100 shadow-2xl relative">
      ...
      <div className="h-16 flex items-center justify-between px-4 border-b border-white/5 bg-slate-900/50">
        ...
        {onClose && (
          <button
            onClick={onClose}
            className="p-2 rounded-lg bg-white/5 border border-white/10 hover:bg-white/10 text-slate-400 hover:text-white transition-all duration-200 cursor-pointer"
          >
            <X className="w-4 h-4" />
          </button>
        )}
        ...
        {logs.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-slate-500 space-y-2 py-8">
            ...
            <p className="text-xs italic">
              {language === 'en' ? 'No messages yet' : "Aucun message pour l'instant"}
            </p>
          </div>
        ) : (
          logs.map((log) => {
            if (log.type === 'system') {
              // Translate system logs to the current local player's language
              const displayContent = translateSystemLog(log.content, language);
                ...
                <div className="flex flex-col space-y-1">
                  <div className={`flex items-center gap-1.5 text-[11px] ${isMe ? 'justify-end' : 'justify-start'}`}>
                    <span className="font-semibold text-slate-300">
                      {log.playerName || (language === 'en' ? 'Player' : 'Joueur')}
                    </span>
                    {isHost && (
      ...
      >
        ...
        <button
          ...
          className="p-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white disabled:opacity-50 disabled:cursor-not-allowed hover:scale-[1.02] active:scale-95 transition-all duration-200 shadow-md flex items-center justify-center cursor-pointer"
        >
