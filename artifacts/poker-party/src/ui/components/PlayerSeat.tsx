  ...
  return (
    <div className={`relative flex flex-col items-center ${!player.active ? 'opacity-50' : ''} ${className}`}>
      ...
      {!isLocal && player.active && !isWaiting && (
        <div className="flex -space-x-4 mb-3 scale-75 origin-bottom opacity-90">
           {player.holeCards.length > 0 ? (
             ...
             // Render 5 face down cards as a placeholder before showdown
      ...
      <div className="relative">
        <div 
          className={`w-16 h-16 rounded-full flex items-center justify-center text-3xl bg-table-panel border-2 shadow-lg transition-all ${
      ...
      <div className="mt-2 text-center flex flex-col items-center">
        <span className="text-sm font-medium text-gray-200 flex items-center gap-1">
          {player.pseudo}
          {player.isHost && <span className="text-[10px] bg-white/10 px-1.5 py-0.5 rounded text-gray-400 uppercase">Hôte</span>}
        </span>
        
        {!isWaiting && player.active && (
          <div className="flex flex-col items-center gap-0.5 mt-1">
            <span className="text-xs font-semibold text-rank-gold">
        ...
        {!player.active && !isWaiting && (
          <span className="text-xs text-red-400 mt-1 italic">Éliminé</span>
        )}
      </div>
    </div>
  );
